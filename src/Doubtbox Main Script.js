function $doubtBox(options) {
  ...
  let mutationTarget = document.getElementById('doubtfulBigChonk'),
    ...
    function mainOps(type, index) {
      ...
      c++;
      //check for missing messages in the loaded section
      var url;
      ...
      fetchData(url, function(messageData) {
        ...
        if (invertFlow) {localMessages.sort((a,b)=>a-b); sortedNew.sort((a,b)=>a-b)} else {localMessages.sort((a,b)=>b-a); sortedNew.sort((a,b)=>b-a)}
        ...
        if (localMessages.length != newSet.length || !(localMessages.every((value, index) => value === sortedNew[index]))) {
          refreshCount = 0;
          if (c>1 && !index) { //not the first loading OR not on a non-zero index. might be able to remove the index check
            console.log('selective')
            // new messages or modifications have been made
            var loadedSet = [];
            $('.doubtboxMessage:not(.doubtbox-ghost)').each(function(){
              loadedSet.push($(this).attr('messageID'))
            })

            //we need to make sure we're performing all modifications, not just one type eg edits, additions, subtractions
            var updateSet = newSet.filter(x => !loadedSet.includes(x)), //message IDs that are not on page: additions
                deleteSet = loadedSet.filter(x => !newSet.includes(x)), //message IDs that need to be removed
                updateObj = messageData.filter(x => updateSet.includes(x.messageID)); //new messages as parseable objects
            ...
            //(additions)compare loadedSet to newSet => updateSet; insert updateSet OBJ at eq(newSet(indexOf(updateID))), then
            //(subtractions)compare newSet to loadedSet => deletionSet; delete $(MID) not included in newSet (deletionSet), then
            //(modifications)compare eq(i).attr('chksum') != =>
            //now delete extraneous messages
                ...
                //handle ghosted elements
                  $('.doubtbox-ghost').each(function(){
                    if ($(this).find('.dbMsgTime').attr('time') == e.timestamp) {
                      ...
                      $newMessage = $(this); //redefine $nM so we can reposition it later
                    }
                  })
                //insert or move the new message to the correct index
            ...
            })//updateObj complete; new messages added
          } else { // if it IS the first load, or if the index is non-zero
          ...
          (c >= 1 && localMessages.length > 0 && type != 'silent') ? $('.doubtbox-audio')[0].play():null //notification
          //end of history != polling
        } else {
          refreshCount++; //no new messages, increment
          console.log('nothing new found;')
        }
        $('.doubtbox-refresh div').removeClass('rotating')
        if (c <= 1) { //first time loading things
          initialization();
        }
...
function scrollyBoy(bigMood) {
  ...
  if (invertFlow) { //inverted direction
...
function markupParser(str) {
  ...
  switch (allowMarkup) {
    ...
    case 'none'://why would you do this?
      for (var i=0;i<parseElem.childNodes.length;i++){
        var a = parseElem.childNodes[i].cloneNode(true);
        if(a.nodeType === 3) {
          fragment.appendChild(document.createRange().createContextualFragment(a.nodeValue))
...
function initialization() {
  ...
  } else if (defaultUsername.length > 0 && '<!-- |g_id| -->' != '2'){ //profile fields return truthy if filled; guest profile fields return the comment.
...
console.log('944')
	function fetchData(url, callback) {
        ...
        (dbH.is(':visible') && $(data).find('form[name="DShoutbox"] p:last a').length) ? dbH.show() : dbH.hide()
				var messageDiv = $(data).find('form[name="DShoutbox"]').children('div:not(.pformstrip)')//first level divs only
                ...
                parsedBody = {};
  				  ...
  				  //This isn't very elegant, I know - but it was fastest in testing by a whole millisecond, and as long as everything in the TOTALLY unordered object is in order, it'll be fine
